...
// Preload fonts and images, then initialize
Promise.all([preloadImages('.intro'), preloadFonts(['Impact'])]).then(() => {
...
const initIntroCanvas = () => {
  // See: https://codepen.io/ksenia-k/pen/dyaeGgO

  const canvas = document.querySelector('#introBackground canvas')
  const image = document.querySelector('#introBackground picture')
  ...
  window.addEventListener('resize', () => {
    params.SPLAT_RADIUS = 3.5 / window.innerHeight
    canvas.width = canvas.clientWidth
    canvas.height = canvas.clientHeight
  })
